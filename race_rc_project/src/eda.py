...
def run_eda(project_root):
    ...
    fig.suptitle(
        "Answer Label Distribution (RACE Dataset)",
        fontweight="bold",
        fontsize=14,
        y=1.02,
    )
    ...
    for i, (name, df) in enumerate(
        ...
        ax.hist(
            ...
            alpha=0.6,
            color=PALETTE[i],
            label=f"{name} (μ={lengths.mean():.0f})",
            edgecolor="none",
        )
    ax.set_xlabel("Number of Words")
    ax.set_ylabel("Frequency")
    ax.set_title(
        "Article Word-Length Distribution",
        fontweight="bold",
        fontsize=13)
    ...
    ax.set_ylabel("Frequency")
    ax.set_title(
        "Question Length Distribution (Training Set)",
        fontweight="bold",
        fontsize=13)
    ax.legend(framealpha=0.3)
    ax.grid(axis="y", linestyle="--")
    ...
    for ax, col, color in zip(axes, ["A", "B", "C", "D"], PALETTE[:4]):
        ...
        ax.grid(axis="y", linestyle="--")
    fig.suptitle(
        "Option Length Distribution (Training Set)",
        fontweight="bold",
        fontsize=13,
        y=1.02,
    )
    ...
    type_counts = {}
    for w in wh_words:
        type_counts[w.capitalize()] = (
            train_df["question"].str.lower().str.startswith(w).sum()
        )
    ...
    ax.set_xlabel("Count")
    ax.set_title(
        "Question Type Distribution (by First Word)",
        fontweight="bold",
        fontsize=13)
    ...
    if os.path.exists(tfidf_path):
        ...
        ax.set_title(
            "Top 20 TF-IDF Terms (Training Articles)",
            fontweight="bold",
            fontsize=13)
        ax.grid(axis="x", linestyle="--")
        fig.tight_layout()
    ...
    for name, df in [("Train", train_df), ("Val", val_df), ("Test", test_df)]:
        ...
        stats.append(
            {
                "Split": name, "Samples": len(df), "Avg Article Length": f"{
                    art_len.mean():.0f}", "Median Article Length": f"{
                    art_len.median():.0f}", "Max Article Length": f"{
                    art_len.max():.0f}", "Avg Question Length": f"{
                        q_len.mean():.1f}", "Answer Distribution": dict(
