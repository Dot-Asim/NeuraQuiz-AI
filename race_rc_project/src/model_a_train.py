...
def load_features(processed_dir: str):
    ...
    X_train, y_train = load_artifact(
        os.path.join(processed_dir, "train_verification_features.pkl")
    )
    X_val, y_val = load_artifact(
        os.path.join(processed_dir, "val_verification_features.pkl")
    )
...
def train_xgboost_gpu(X_train, y_train, X_val, y_val):
    ...
    print(
        f"  Class imbalance: {neg_count}:{pos_count} -> scale_pos_weight={spw:.2f}")
...
def train_pytorch_mlp(
    X_train, y_train, X_val, y_val, epochs=50, batch_size=1024, lr=1e-3
):
...
def train_logistic_regression(X_train, y_train, X_val, y_val):
    ...
    model = LogisticRegression(
        max_iter=3000, C=0.5, solver="lbfgs", class_weight="balanced"
    )
...
def train_svm(X_train, y_train, X_val, y_val):
    ...
    if len(X_train) > max_svm_samples:
        ...
        print(
            f"  Using subsample: {max_svm_samples}/{len(X_train)} (SVM is O(n^2))")
    else:
        X_tr_sub = X_train
        y_tr_sub = y_train
...
def train_label_propagation(
    ...
    print("\n" + "=" * 60)
    print(
        f"  Training: Label Propagation (Semi-Supervised, {unlabeled_fraction * 100:.0f}% unlabeled)"
    )
    print("=" * 60)
    start = time.time()
    ...
    if len(X_tr) > max_samples:
        ...
        print(
            f"  Using subset: {max_samples} samples (LP is memory-intensive)")
    else:
        X_tr_sub = X_tr
        y_semi_sub = y_semi
...
if __name__ == "__main__":
    ...
    # 3. GPU K-Means
    km_centroids, km_scaler, km_res = train_gpu_kmeans(
        X_train, y_train, n_clusters=2)
    results["KMeans_GPU"] = km_res
    save_artifact((km_centroids, km_scaler),
                  os.path.join(MODEL_DIR, "kmeans_gpu.pkl"))

    # ---- CPU Models (fast, for ensemble diversity) ----
    lr_model, lr_res = train_logistic_regression(
    ...
    lp_model, lp_scaler, lp_res = train_label_propagation(
        X_train, y_train, X_val, y_val
    )
    results["LabelPropagation"] = lp_res
    save_artifact(
        (lp_model, lp_scaler), os.path.join(MODEL_DIR, "label_propagation.pkl")
    )
    ...
    ens_names, ens_res = build_gpu_ensemble(
        ensemble_models, X_val, y_val, mlp_model=mlp_model, mlp_scaler=mlp_scaler)
